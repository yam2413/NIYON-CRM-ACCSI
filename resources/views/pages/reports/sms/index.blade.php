					...
					<div class="card card-custom">
						...
						<div class="card-body">
							 <p class="mb-0"><span class="card-icon">
								...
								</span>This table shows the sms logs in the crm.
							 </p>
							...
							<!--begin: Datatable-->
							<table id="tbl_sms_logs" class="table table-bordered table-hover table-checkable">
								<thead>
									<tr>
										<th>Borrower Name</th>
										<th>To</th>
...
var arrows;
 ...
 $('#call_date .form-control').val(date);
 	...
 	jQuery(document).on('click', '#export_reports', function(e){
    	...
    	var excel = '{{ route('pages.reports.sms.export_sms_logs', ['date' => ':date']) }}';
	...
	function func_table_reports(date){
		var tbl_sms_logs = $('#tbl_sms_logs');
		tbl_sms_logs.DataTable({
			responsive: true,
			searchDelay: 500,
			processing: true,
			serverSide: true,
	...
	$('#call_date').daterangepicker({
            ...
            $('#call_date .form-control').val(date);
            $('#tbl_sms_logs').DataTable().destroy();
            func_table_reports(date);
            DEMOGRAPH_GROUPS(date);
    });	
