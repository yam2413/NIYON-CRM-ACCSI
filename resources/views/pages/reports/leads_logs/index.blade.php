...
@section('title', 'Leads Log Reports')
@section('content')
<!--begin::Entry-->
				...
				<div class="col-lg-12">
					...
					<div class="card card-custom">
						...
						<div class="card-body">
										...
										<!--begin::Body-->
											<div class="card-body d-flex align-items-center justify-content-between flex-wrap">
												<div class="mr-2">
													...
													<div class="text-dark-50 font-size-lg mt-2">This table shows the leads logs in the crm. it contains the Add, Edit or Delete action.</div>
												</div>
							...
							<!--begin: Datatable-->
							<table id="tbl_leads_logs" class="table table-bordered table-hover table-checkable">
								<thead>
									<tr>
										<th>Borrower Name</th>
										<th>Actions</th>
...
var arrows;
 ...
 $('#call_date .form-control').val(date);
 	...
 	jQuery(document).on('click', '#export_reports', function(e){
      ...
      var status 	  	= 0;
        ...
        $.ajax({
		        ...
		        type: "POST",
		        data: 'report_type=leads_logs&groups='+groups+'&collector='+collector+'&status='+status+'&date='+date,
		        headers: {
	...
	function func_table_reports(collector,date){
		var tbl_leads_logs = $('#tbl_leads_logs');
		tbl_leads_logs.DataTable({
			responsive: true,
			searchDelay: 500,
			processing: true,
			serverSide: true,
			searching: true,
			ajax: {
				    url: "/leads_logs/getLeadsLogsList",
				    type: 'POST',
				    headers: {
	...
	jQuery(document).on('change', '#filter_collector', function(e){
      ...
      var date = $('#call_date .form-control').val();
	   ...
	   $('#tbl_leads_logs').DataTable().destroy();
