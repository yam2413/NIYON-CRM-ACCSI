			...
			<div class="flex-row-auto offcanvas-mobile w-200px w-xl-250px" id="kt_profile_aside">
				...
				<div class="card card-custom gutter-b">
					...
					<div class="card-body pt-4">
						...
						<!--end::Contact-->
						<a href="{{ route('pages.agent.auto_dialer', ['file_id' => $file_id]) }}" class="btn btn-secondary font-weight-bold py-3 px-6 mb-2 text-center btn-block">
						Go Back to Auto Dialer
						</a>
...
window.clockTick = function(){
  ...
  data =
	...
	get_call_status_view_auto = function(contact_no){
		jQuery.ajax({
	...
	function start_call(call_contact_no, name, profile_id, leads_id){
		KTApp.unblockPage();
			...
			Swal.fire({
		        ...
		        if (result.value) {
					...
					$.ajax({
					    ...
					    success: function(data){
					       KTApp.unblockPage();
					       intervalID = setInterval(function() { get_call_status_view_auto(call_contact_no); }, 5000);
				  			...
				  			$("#btn-campaign-pause").prop('disabled', false);
								...
								switch(account_status) {
								  case 0:
								    ...
								    window.start();
					  				if(dial_status == 0){
							  			start_call(contact_no, name, profile_id, leads_id);
						  			}else{
						  				intervalID = setInterval(function() { get_call_status_view_auto(last_call_contact_no); }, 5000);
						  				//$("#btn-call-again").html('Call Again?');
						  			}
								  ...
								  case 1:
								    ...
								    window.start();
								    if(dial_status == 0){
							  			start_call(contact_no, dname, profile_id, leads_id);
						  			}else{
						  				intervalID = setInterval(function() { get_call_status_view_auto(last_call_contact_no); }, 5000);
						  				//$("#btn-call-again").html('Call Again?');
						  			}
	...
	function get_my_leads(){
		...
		jQuery.ajax({
			url: "/previous_data/get_previous_leads_data",
			type: "POST",
			data: 'user_id={{Auth::user()->id}}&file_id={{$file_id}}&leads_id={{$leads_id}}',
			headers: {
				'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
			},
			dataType: "json",
