...
class PulloutAccountsExport implements FromCollection, WithHeadings, WithMapping
{
    /**
    * @return \Illuminate\Support\Collection
    */
    public $id, $user_id, $uniq_id, $group_id, $type;
    public function __construct($id, $user_id, $uniq_id, $group_id, $type)
    {
        $this->id  		= $id;
    ...
    public function collection()
    {
	    ...
	    //$records->leftjoin('crm_ptp_histories', 'crm_ptp_histories.leads_id', '=', 'crm_leads.id');
	    //$records->leftjoin('temp_uploads', 'temp_uploads.profile_id', '=', 'crm_leads.profile_id');
	    ...
	    switch ($this->type) {
    		case 'import':
                ...
                $records->whereRaw('crm_leads.account_number IN (SELECT data1 FROM temp_uploads WHERE file_id = "'.$this->uniq_id.'" )');
    			//$records->where('temp_uploads.file_id','=' ,$this->uniq_id);
    			break;
    		...
    		case 'all':
    			...
    			$records->where('crm_leads.assign_group','=' ,$this->group_id);
    			break;
	    ...
	    foreach ($records as $key => $record) {
	        ...
	        switch ($status) {
	     	...
	     	SystemLogs::saveLogs($this->user_id, $full_name.' has been pullout in the leads');
	     	CrmLeads::where('id', '=', $id)->delete();
	     	CrmBorrowers::where('profile_id', '=', $profile_id)->delete();
	     	CrmPtpHistories::where('leads_id', '=', $id)->delete();
	     	AddedCampaignsLeads::where('leads_id', '=', $id)->delete();
	     	CrmLogs::where('profile_id', '=', $profile_id)->delete();
	     	CrmCallLogs::where('profile_id', '=', $profile_id)->delete();
	     	TempUploads::where('profile_id', '=', $profile_id)->where('file_id', '=', $record->file_id)->delete();
	     	ManualNumbers::where('profile_id', '=', $profile_id)->delete();
	    }
